...
describe SessionProxy do
  ...
  describe '#response_from_rp' do
    it 'should return an rp response' do
      expect(api_client).to receive(:get)
        .with(SessionProxy::RESPONSE_FROM_RP_PATH, cookies: cookies)
        .and_return(
            'location' => 'http://www.example.com',
            'samlRequest' => 'a saml message',
            'relayState' => 'a relay state'
        )

      actual_response = session_proxy.response_from_rp(cookies)
    ...
    it 'should raise an error when the API responds with an unknown value' do
      ...
      expect {
        session_proxy.response_from_rp(cookies)
